...
# JSON 파일 경로
json_file_path = 'FaceForensics++.json'

json_data = {"FaceForensics++": {}}
...
for csv_file_path in csv_file_paths:
    ...
    with open(csv_file_path, mode='r', encoding='utf-8') as file:  # Open csv file
        ...
        for data in csv_reader:
            ...
            # json 파일에 데이터 추가
            if df_type not in json_data['FaceForensics++']:
                json_data['FaceForensics++'][df_type] = {}
            if type_data not in json_data['FaceForensics++'][df_type]:
                json_data['FaceForensics++'][df_type][type_data] = {}
            if 'c23' not in json_data['FaceForensics++'][df_type][type_data]:
                json_data['FaceForensics++'][df_type][type_data]['c23'] = {}
            if video_key not in json_data['FaceForensics++'][df_type][type_data]['c23']:
                json_data['FaceForensics++'][df_type][type_data]['c23'][video_key] = {
                    "label": df_type,
                    "frames": []
                }
            ...
            json_data['FaceForensics++'][df_type][type_data]['c23'][video_key]['frames'].append(frame_path)
            if new_video:
                if 'fake' in csv_file_path:
                    json_data['FaceForensics++'][df_type][type_data]['c23'][video_key]['attributes'] = {
                        'ismale': data['ismale'],
